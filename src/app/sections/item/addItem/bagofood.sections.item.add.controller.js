...
angular.module('bagofood.sections.item.add.controller', ['bagofood.core.service.foodlist'])
    .controller('AddItemController', function($scope, $stateParams, $modal, $log, $previousState, ngTableParams, ProductService, CONST, FoodListService) {
        ...
        $scope.item = angular.copy($stateParams.item) || {
            category: CONST.itemCategories[0]
        };
        ...
        $scope.ok = function() {
            ...
            if (typeof $scope.item.id === 'undefined') {
                FoodListService.addItemOnFoodList({
                    'id': $stateParams.foodListId
                }, $scope.item).$promise.then(
                    function(data) {
                        $scope.back();
                    });
            } else {
                FoodListService.updateItemOnFoodList({
                    'id': $stateParams.foodListId,
        ...
        $scope.searchProductCiqual2012 = function($defer, params) {
            ...
            if ($scope.productCiqual2012) {
                ...
                ProductService.getFoodListByOrigfdnm({
                    'origfdnm': $scope.productCiqual2012

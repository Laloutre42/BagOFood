  ...
  .controller('AddItemController', function ($scope, $stateParams, $previousState, CONST, FoodListService) {
    ...
    $scope.ok = function () {
      ...
      if (typeof $scope.item.id === 'undefined') {
        FoodListService.addItemOnFoodList({'id': $stateParams.foodListId}, $scope.item).$promise.then($scope.back());
      }
      else {
        FoodListService.updateItemOnFoodList({'id': $stateParams.foodListId, 'itemId': $scope.item.id}, $scope.item).$promise.then($scope.back());
      }
    };
